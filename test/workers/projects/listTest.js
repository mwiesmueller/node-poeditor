...
describe('projects.list...', () => {
  it('... is of type function', (done) => {
    assert.that(workers.projects.list).is.ofType('function');
    done();
  });
  ...
  it('... must fetch the api error messages', (done) => {
    (async () => {
      try {
        await workers.projects.list('notfamoustoken');
      } catch (err) {
  ...
  it('... resolves a object when process is done', (done) => {
    (async () => {
      try {
        const res = await workers.projects.list(processenv('API_TOKEN'));
