...
describe('terms.add...', () => {
  it('... is of type function', (done) => {
    assert.that(workers.terms.add).is.ofType('function');
    done();
  });
  ...
  it('... rejects an error when function is called without a id for the project', (done) => {
    (async () => {
      try {
        await workers.terms.add('abcdef');
      } catch (err) {
  ...
  it('... rejects an error when function is called without data to add', (done) => {
    (async () => {
      try {
        await workers.terms.add('abcdef', '1234');
      } catch (err) {
  ...
  it('... rejects an error when function is called data are not of type object', (done) => {
    (async () => {
      try {
        await workers.terms.add('abcdef', '1234', 'string');
      } catch (err) {
        assert.that(err).is.equalTo('Error: You must define a valid data object');
        done();
      }
  ...
  it('... must fetch the api error messages', (done) => {
    (async () => {
      try {
        await workers.terms.add('notfamoustoken', '1234', []);
      } catch (err) {
  ...
  it('... must resolve all terms from the project when process is done', (done) => {
    (async () => {
      try {
        ...
        const res = await workers.terms.add(processenv('API_TOKEN'), add.project.id, data);
