...
describe('languages.add...', () => {
  it('... is of type function', (done) => {
    assert.that(workers.languages.add).is.ofType('function');
    done();
  });
  ...
  it('... rejects an error when function is called without a project id', (done) => {
    (async () => {
      try {
        await workers.languages.add('1234');
      } catch (err) {
  ...
  it('... rejects an error when function is called without a valid languagecode', (done) => {
    (async () => {
      try {
        await workers.languages.add('1234', 'abc');
      } catch (err) {
  ...
  it('... must fetch the api error messages', (done) => {
    (async () => {
      try {
        await workers.languages.add('notfamoustoken', '12345', 'de');
      } catch (err) {
  ...
  it('... must resolve true when process is done', (done) => {
    (async () => {
      try {
        ...
        const res = await workers.languages.add(processenv('API_TOKEN'), add.project.id, 'de');
